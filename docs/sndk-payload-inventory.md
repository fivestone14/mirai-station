...
`sndk_read.read_once` (ERA sr-7), chain = `sndk_feed.sndk_chain`.
...
| scale.expected_move_today_asym {up/down_dollars, skewed_toward, derived_from} | `range_ruler.em_points` split by `iv_skew.down_share` | **IV-skew split** (blueprint semantics); the pre-existing `adaptive_em` (realized-semivariance split) stays recorded on the row as shadow — divergence flagged in the 08-02 report |
...
| price.vwap_dist_sigma_from_live_spot | `vwap` (Schwab 1-min bars, Σp·v/Σv) vs LIVE spot on σ | the only σ distance in the scene measured off the live quote — VWAP is a live-tape level, so its name says which ruler it used |
...
| `vwap` (raw $) | ships as `price.vwap_dist_sigma_from_live_spot` (scale-free) |
