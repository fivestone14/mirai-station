...
| price.live_spot / spot_when_book_was_measured / ~~live_minus_book_spot_dollars~~ / live_minus_book_spot_sigma / vs_prior_close_pct / session_low / high (+ `_at` and `_min_ago` on each, strikes-3, item #11) / moved_last_30min_sigma | `spot`, `meta.chain_spot`, `prior_close`, day path over rows | **sr-7 (08-30)**: `meta.chain_spot` was on the row and dropped. The two spots differ by a median $2.13 on cached rows and by $41.99 at worst; `live_minus_book_spot_sigma` is the one number that converts any book-measured σ distance into a distance from where price actually is — SUBTRACTED, not added. The doctrine said "add" for one afternoon, which walks the wrong way by twice the gap (0.18σ on the 08-28 11:04 row, ~0.8σ on the worst recorded row). The field was renamed to match its dollar twin so the direction rides on the name. **sr-8 (08-30)**: the dollar twin itself DELETED — the same quantity in the other unit, with both spots it differences in the same block. Sigma is the frame every other distance is already in, so sigma is the copy that survives |
| price.vwap / price.vwap_minus_live_spot_sigma | `vwap` (Schwab 1-min bars, Σp·v/Σv), as a level and as a σ distance from the LIVE spot | **`price.vwap` ADDED 2026-09-13 (review item #22)**: the average rode on the board only as a σ distance, and a distance cannot be named or pointed at — the model said it exactly zero times across 538 recorded readings, and a figure worked back out of the sigma was deleted as a level not on the board two times in three. The price now ships beside the distance for 15 characters and is admitted as a pointable level; measured present on 292 of 292 rebuilt scenes. The distance stays: it is the only σ distance in the scene measured off the live quote — VWAP is a live-tape level. **RENAMED sr-8 (08-30)** from `vwap_dist_sigma_from_live_spot`, which said which ruler it used and never which way it pointed: it is (vwap − live spot)/σ, so POSITIVE means vwap ABOVE price — price BELOW its average — the opposite sense to `moved_last_30min_sigma`. 13 of 15 reviewers read the sign backwards. The phone glance dodged it by rendering the vwap PRICE; the scene ships σ distances and cannot, so the name now carries the subtraction, matching `live_minus_book_spot_sigma` |
...
| momentum {window_between_books {books_compared, span_min, first_book_measured_at, ~~last_book_measured_at~~}, by_strike share_of_book_gamma_change_pp / gross_volume_change_contracts} | `mass_by_strike` share deltas + `vol_gross_by_strike` deltas over the last 5 scans, intersection-denominated, window told on the BOOK clock | **sr-8 (08-30)**: `last_book_measured_at` DELETED — it is the current book, and equalled `data_sources.options_book.measured_at` on 4,026/4,026 scans carrying momentum; the far end is the one a reader cannot get anywhere else. `oi_d` **omitted**: upstream OI updates once daily — an intraday oi_d is a permanent 0 (measured 07-30: OI@1300 unique value all day). `cvd` **omitted**: no bid/ask aggressor tape exists for SNDK stock (1-min bars only; a bar-direction proxy measures drift, not aggression — the 07-17 BVC lesson) |
...
| **context.ranges** {opening {low, high, formed_over, status \| still_forming}, in_force {low, high, froze_at, standing_for_min, is_the_opening_box (strikes-3, item #13: was formed_over + live_spot_is) \| still_forming, since, replaced_a_box_broken_at}, breaks_today {count, breaks [{at, went, box_low, box_high, back_inside_at (strikes-3, item #12)}]}, prior_sessions {sessions, from, to, low, high, measured_from, live_spot_is, today_traded_beyond_it, todays_opening_box_is} — LEGACY SCENE ONLY since 2026-09-13, review item #34: it shipped on every call and none of the 119 recorded readings used it} | today's scans + the prior 5 sessions' diary files (never today's own) | **NEW obs-4 (09-02)**: the day as boxes. Opening box = first 30 min; a frozen box breaks past 0.05σ (strikes-3: past the move bar, two typical minutes as they stood at that minute; no minute record, no break); a break starts a new box that forms over 30 min. LIVE-TAPE sourced, so it survives a stale book. Box edges are admitted as pointable levels. |
| history {~~price_at_level_unseen_earlier_today~~, tape_abnormal_vs_own_history} — LEGACY SCENE ONLY since 2026-09-13: `history` came off the board's `KEPT_BLOCKS` because the builder popped its only surviving flag four lines later, so it reached the reading model empty on 880 of 880 rebuilt boards and was then dropped for being empty | today's rows (spot beyond all prior scans = new session ground; ≥1.5σ day move on the stock's OWN ruler or ≥0.5σ/30min = abnormal) | the under-pull flags; block omitted when nothing to say. σ-relative on purpose: a fixed 8% is ~1σ on this name and fired on 56% of recorded scans. `price_at_level_unseen_earlier_today` DELETED at 25d1bc6: it fired 16 times across 8 replayed sessions and the payload's own extremes contradicted all 16, because it asked the 2-minute scans while `session_high`/`session_low` answer from the 1-minute bars. The legacy scene the voice reads still carries the block, measured on 37 of 292 rebuilt scenes |
...
| ~~`vwap` (raw $)~~ | **NO LONGER OUT — 2026-09-13, review item #22**: it ships as `price.vwap`, the dollar level itself, beside the scale-free `price.vwap_minus_live_spot_sigma`. Scale-free was the wrong test for this one: a distance cannot be named, and the model never once said it |
...
figure, from 2026-09-10), `strikes_sent_without_volume` (true when that
list was drawn with the book's volume withheld, item #8), `strikes_sent` (the strikes the Strikes Payload's
