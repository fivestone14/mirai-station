...
def schema_section():
    ...
    files = [
        ("state/jev/{day}.jsonl", "every run", "row_ts, sigma, event (a tier-1 scheduled event due within the hour, or null; never sent to JEV), state (the labels), omitted, requests, skipped, held {qid: since}, cadence_from, answers (JEV's raw replies), hour"),
        ...
        ("state/jev/grades.jsonl", "one line per graded mark", "row_ts, horizons (which sums this line grades), pending, skipped, then per sum: realized_sigma, band, pick, hit, brier (the blend), jev_pick, jev_hit, jev_brier, clock_brier; fresh; event_within_30 and event when a scheduled event was due; a halted-window line for a mark whose bars never came on a finished day; or graded false with the reason, for a read that can never be graded"),
        ...
        ("state/jev/weights.json", "what step 3 reads", "graded_runs, sums {next_30, next_60: n, hit_rate, mean_brier, blended {mean_brier_blend, mean_brier_jev, mean_brier_clock}, event_reads {n, mean_brier}}, questions {qid: {weight, mi (the tracking score, see below), n, in_step_3, why}}, live questions only"),
...
<div class="rv"><code class="qid">safety checks</code><span class="opt">code, live runs</span><span class="why">a sent read on a row more than 6 minutes old is skipped and the last card stays; an options book more than 6 minutes old at the read has every label built from it left out; a side packet whose own checks failed is not used; a tier-1 scheduled event due within the hour tags the read (calendar/events.json), and one due within 30 minutes is graded but never pairs into a question's weight</span></div>
...
def weight_log_section():
    ...
    # the weight log keeps retired questions' history; the strip names only questions still asked
    top = sorted(((q, v) for q, v in moved.items() if q in ask_of), key=lambda kv: -kv[1])[:5]
