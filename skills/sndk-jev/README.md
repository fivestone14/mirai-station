...
| `runtime/scripts/run-sndk-jev.sh`, `runtime/launchd/com.mirai-station.sndk-jev.plist` | The Job | The launchd wrapper (beside the station's other runners, where its job test expects it) and the plist. Installed on the station since 2026-09-22; fires at :02 and :32 ET, so JEV reads once per half hour in market hours. `com.mirai-station.sndk-jev-tape.plist` runs the same wrapper with `--lane tape`, twelve fires 09:35 to 10:30 ET and a close-out at 10:42 that asks JEV nothing, grades the last two calls and refreshes the card (`lane.TAPE.schedule`, `close_out`; a test holds the plist to them), its own log files. |
| `runtime/viewstation/static/m/jev.html` | The Card | The phone page. Polls `latest.json` every 20 seconds, and 12 seconds after each :02 and :32 read, through the viewstation's existing read-only raw-file route, so `server.py` is unchanged. It also polls the opening lane's card (`state/jev/lanes/tape/latest.json`) in market hours. Two modes, switched by the clock and the cards alone: while today's newest lane call is still open, the 5-minute lane's 10-minute call leads and the 30-minute call folds to one line; once it closes (10:40 after the 10:30 read, sooner if the lane stops) the 30-minute call leads again and the opening folds to its score for the day. Each leading call opens with a clock (how far ahead it looks, minutes left, the minute it is graded) and the calls in play: the newest calls on the card (`calls`, from `service.day_calls`) drawn on one clock from read to mark, so two open at once, the overlap, shows; graded ones say what happened. The page counts minutes against the marks and computes no grade. |
...
- `latest.json`, the phone's card: `symbol`, `generated_at`, `row_ts`,
  ...
  follow a 13:00 half day), `calls` (the day's newest four sums, newest first:
  `read`, `mark` (the read's minute plus the horizon, as the grader counts it),
  `minutes`, `pick`, `p`, `odds` (every option as the phone showed it), then
  `outcome`, `hit` and `moved` (the realized move behind the grade) once graded
  or `closed` with the reason; the phone opens a sheet with these on a tap), `tally` (the day's `calls`, `graded` and `right`), on the tape
  lane's card also `lane`, `ruler`, `band`, `stretch`, `schedule` and, after the
  10:42 close-out, `closed_out_at`,
