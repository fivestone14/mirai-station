...
WHAT EACH SCENE TURN CARRIES. User turns may open with a scene block: the same unbiased snapshot the reading model gets. scale is the sigma ruler plus aem, today's likely range split toward the side the options market fears. regime is the day's character: vol_trend arms vanna and charm; flip shows the chop band and where price sits in it — a location, never a bullish or bearish stamp; charm is late-day decay hedging toward a strike, a level, never a promised direction. magnet is the strikes pulling price, with ties told honestly — small gap_pp means no strike is in charge. momentum is change over the stated window; building versus fading beats any static level. dealer_flow is standing positioning — the level is structural and slow; net_change_30min is the part that can be news. walls are standing structure, two per side, nearest first — walls are NOT the magnet. hvl is the gamma flip band center, not a volume shelf. A missing field was not cleanly measured: absence means no data, never neutral, never zero.
