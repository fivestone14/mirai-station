#!/usr/bin/env python3
# Interpreter: the station venv (the embedder lives there); the shebang cannot
# pin it, so name the venv python explicitly when running by hand.
"""
lefteye_rag.py — the SPX Watchtower's on-demand memory: day tier + month tier,
as a TOOL.

This is the sndk_rag.py v2 shape, templated back onto the SPX station (wt-11,
2026-08-10). The two modules are deliberate twins — a retrieval fix proven on
one tape should be ported to the other, and their record shapes are kept
parallel on purpose (RAG_V tracks the SHARED shape era, so both stores speak
the same schema language).

CORE RULE (the sr-2 blueprint, section 05): history is a callable tool the
reasoning model reaches for when the live scene does not add up — it is NEVER
...
WHAT ONE RECORD IS — one entry, two faces:
  Face A, structured metadata (the GATE): time, levels, gamma, walls, sigma,
     and the vector + magnitude the tower gave. Exact-filterable, NOT embedded
     (numbers embed poorly — similarity blurs strikes — but filter precisely).
  Face B, narrative (the MATCH): the tower's OWN scene sentence for that
     judged scan. This face is what gets ranked by meaning at query time.

TIERS:
  day    state/spx_rag/slices/{date}.jsonl — one slice per judged verdict,
         plus the DIARY (state/reversion) as the raw numeric series the
         `series` command reads. Slices roll up into a day summary.
  month  summaries.jsonl (one row per session) + terrain.json (standing walls,
         recurring battle lines, tape character) — context, never the trigger.
...
WHAT A SLICE MAY NOT CARRY (the 08-02 adversarial-audit rule, ported): the
FADE GATES' verdict. The tower's own past vector is sanctioned memory; Head
A's fired/armed decision is exactly the pre-baked conclusion the blind pass
exists to hide, and slices are one allow-listed call away at the uncertain
moments where anchoring bites. Same rule for interest strings that name the
gates — neutralized to "gate event".

Isolation: writes ONLY under state/spx_rag/. Reads the SPX reversion diary;
never touches any SNDK store. Called two ways:
  * imported by watchtower (record_slice — fail-open, must never cost a scan)
  * as the allow-listed CLI the reasoning model may run (watchtower._RAG_CMD)

CLI:
  lefteye_rag.py query --tier slices  [--date D] [--from HH:MM --to HH:MM]
                    [--near-strike K [--tolerance PTS]] [--text "..."] [--limit N]
  lefteye_rag.py query --tier days    [--days-back N] [--date D]
                    [--from-date D --to-date D] [--min-move ±PCT]
                    [--near-strike K [--tolerance PTS]] [--text "..."]
  lefteye_rag.py query --tier month
  lefteye_rag.py series [--date D] [--from HH:MM] [--to HH:MM] [--step MIN]
  lefteye_rag.py rollup [--date D] [--force]      # manual; queries auto-rollup
"""
...
RAG_V = 2                    # the SHARED record-shape era (twin of sndk_rag) —
                             # this store is born at v2: stitched day
                             # narratives, terrain recent/all_time split,
                             # days-tier Face-A filters. Bump BOTH modules on
                             # any shape change.
...
NEAR_TOL_PTS = 25.0          # default "near a strike" tolerance, $ (5 grid
                             # steps on the SPX 5-point ladder; SNDK uses 15
                             # on its $5 grid — each tape gets its own ruler)
...
def _top_strikes(row: dict) -> list[list[float]]:
    """The magnet band's top two strikes as [strike, share-of-mass %] — the
    same tie-honesty read the payload carries, recorded so a slice can be
    found by the strike that was actually in charge (or contesting)."""
    mb = []
    for pair in ((row.get("gex_views") or {}).get("mass_by_strike") or []):
        try:
            k, v = _num(pair[0]), _num(pair[1])
    ...
    if not mb or tot <= 0:
        return []
    ranked = sorted(mb, key=lambda x: -x[1])[:2]
    return [[round(k, 2), round(v / tot * 100.0, 2)] for k, v in ranked]


# ---------------------------------------------------------------------------
# writing — one slice per judged verdict (called from watchtower, fail-open)
# ---------------------------------------------------------------------------
def record_slice(row: dict, wt: dict, now: datetime) -> None:
    """One RAG entry: what was true (Face A) + what the tower read (Face B).

    Face B is the tower's OWN scene sentence — the judging already happened,
    so memory costs no extra call. Raw numeric series are NOT duplicated here
    (the diary is the plain store `series` reads); this file holds only the
    memorable-narrative record."""
    line = wt.get("scene")
    if not line:
        return                      # no sentence → nothing memorable to store
    narrative = str(line)
    if wt.get("would_change_mind"):
        narrative += f" Changes if {wt['would_change_mind']}"
    ladder = row.get("profile_ladder") or {}
    fired = wt.get("fired") is True
    d = wt.get("direction")
    vector = ("up" if d == "call" else "down" if d == "put" else "none") \
        if fired else "none"
    meta = {
        ...
        "magnet_top": _top_strikes(row) or None,
        "walls_call": ([w] if (w := _num(row.get("call_wall"))) is not None
                       else None),
        "walls_put": ([w] if (w := _num(row.get("put_wall"))) is not None
                      else None),
        "ct": _num(ladder.get("ct")), "hvl": _num(ladder.get("hvl")),
        "pt": _num(ladder.get("pt")), "vwap": _num(row.get("vwap")),
        "vector": vector,
        "magnitude_sigma": _num(wt.get("magnitude_sigma")),
        "stance": wt.get("stance"),
        # HEAD A'S VERDICT IS DELIBERATELY NOT STORED (the 08-02 audit rule,
        # ported): the tower's own past vector is sanctioned memory; the fade
        # gates' decision is the pre-baked conclusion the blind pass hides.
        # Same rule for interest strings that name the gates.
        "wake": ("gate event"
                 if str(wt.get("interest") or "").startswith("gates")
                 else wt.get("interest")),
    }
    rec = {"ts": now.isoformat(), "kind": "slice", "rag_v": RAG_V,
           "era": wt.get("prompt_version"),
...
def _diary_day(day: str) -> list[dict]:
    ...
    SPX rows carry no forced flag today, but the guard rides anyway (the
    SNDK store learned this the hard way: a midnight warmup row narrated as
    the session open). Off-RTH rows are excluded on the timestamp."""
    out = []
...
def _story_bit(narrative: str) -> str:
    """One slice sentence, trimmed for a day summary: the tower's line only —
...
def _slice_time(s: dict) -> str:
    ...
    time must never crash a rollup)."""
...
def _pick_story_slices(slices: list[dict], diary: list[dict]) -> list[dict]:
    """The day's arc in ≤3 of the tower's OWN sentences: the first read, the
...
def build_summary(day: str) -> Optional[dict]:
    ...
    Slices preferred (they carry the tower's reads); the diary alone still
    yields an honest pre-RAG summary (source says which). None = no data."""
    slices = _read_jsonl(_slices_path(day))
    # same live-tape rule the diary side applies: an off-hours read must not
    # narrate the session
    ...
    if vecs:
        ...
        if top in ("up", "down") and n > vecs.get("none", 0):
            lean = top
    # ±1% open-to-close is a MOVED day on SPX (σ runs ~0.5-0.8% of spot) —
    # the same wording bar SNDK uses happens to be the right one here too
    ...
    bits = [f"SPX closed {abs(pct):.1f}% {'below' if pct < 0 else 'above'} its open"
            if pct else "SPX closed near its open",
            f"ranged {lo:g}–{hi:g}",
            ...
            f"the tower leaned {lean}" if lean else None]
    # spine first — the numbers stay authoritative, and the ISO date prefix
    # gives exact tokens no prose date can (embeddings rank neither dates nor
    # magnitudes; the spine is the honest core, the stitched story below is
    # the searchable/readable face)
    ...
    # moment, never presented as the day's verdict, and the exact HH:MM
    # tokens stay searchable
...
def rollup(day: Optional[str] = None, force: bool = False) -> list[str]:
    ...
    sits inside the reasoning model's Bash grant, and a mid-session summary
    would freeze half a day as "the day" (and poison terrain counts) since
    idempotency keeps first writes forever."""
    day = _ymd(day)
    ...
    if day:
        ...
        days = sorted({p.stem for p in _diary_dir().glob("2*.jsonl")}
    ...
    for d in days:
        ...
        if not s:
            ...
            # stale summary — a day with no live tape is not a session.
    ...
    if done:
        ...
        # + replace — two parallel queries may both auto-rollup, and a SHARED
        # tmp name lets one replace the other's file out from under it
...
def terrain(rebuild: bool = False) -> Optional[dict]:
    ...
    if not rebuild:
        try:
            ...
            # same day AND same schema era — a stale-era terrain built this
            # morning must not be served in the old shape all day
    ...
    # RECENT vs ALL-TIME: most_common(3) over the whole window lets old levels
    # crowd out the walls price is actually trading against now — the headline
    # speaks from the recent map, the all-time counts stay alongside for depth.
...
def _chunks(narrative: str) -> list[str]:
    """A narrative's sentences, for max-scoring. Ranking whole narratives lets
    story-rich rows blanket-outrank short rows on sheer vocabulary — a
    sentence can only ADD match signal, never pad it."""
...
def _rank(query: str, records: list[dict]) -> tuple[list[dict], str]:
    ...
    try:
        ...
        # progress bars would leak into the reasoning model's tool output
...
def _near_day(meta: dict, strike: float, tol: float) -> bool:
    ...
    (range containment — "has 7900 held before?" must match a day that
    printed 7900 mid-range), or touch it with its endpoints or modal
    magnet/walls?"""
...
def query(tier: str = "slices", date: Optional[str] = None,
    ...
    if tier == "days":
        rollup()
        cands = _summaries()
        # Face-A filters FIRST (embeddings cannot rank dates or magnitudes —
        # exact asks go through the numbers). A filtered ask searches ALL
        # history, not the last N — --date 3 weeks back must not be silently
        # cut by the days_back page.
        ...
        if min_move is not None:
            # signed: --min-move -1 → sessions that fell ≥1%; 1 → rose ≥1%;
        ...
        if not filtered:
            ...
            # "has 7900 held before?" must reach further back than a week,
...
# ---------------------------------------------------------------------------
# CLI — what the reasoning model actually runs
# ---------------------------------------------------------------------------
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(
        prog="lefteye_rag.py",
        description="SPX history tool — day slices, day summaries, month "
    ...
    q.add_argument("--min-move", type=float,
                   help="days tier, signed %% open-to-close: -1 → sessions "
                        "that fell ≥1%%, 1 → rose ≥1%%, 0 → any session "
