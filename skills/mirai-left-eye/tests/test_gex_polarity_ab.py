...
def test_promotion_gate_watchtower_reads_its_own_ledger(tmp_path):
    # Head B is judged on the "watchtower" sub-record, never on Head A's
    hist = tmp_path / "hist.jsonl"
    lines = [_day_line(f"2026-07-{i:02d}", 3, 4, key="watchtower") for i in range(1, 14)]
    lines += [_day_line(f"2026-08-{i:02d}", 1, 4, key="watchtower") for i in range(1, 4)]
    hist.write_text("\n".join(lines) + "\n")
    assert pab.promotion_gate(hist=hist)[0] is False                 # lens: no record
    assert pab.promotion_gate(hist=hist, record_key="watchtower")[0] is True


def _wt_row(minute, gates_fired, tower_fired, direction="call", tower_dir=None,
            interest=None, scene=None):
    """A diary row where both heads spoke: the gates via reversion_extreme,
    the tower via its watchtower sub-record."""
    ts = (T0 + timedelta(minutes=minute)).isoformat()
    wt = {"fired": tower_fired,
          "direction": (tower_dir or direction) if tower_fired else None,
          "votes": {"n": 3, "agreement": 1.0}}
    if interest is not None:
        wt["interest"] = interest
    if scene is not None:
        wt["scene"] = scene
    return {"ticker": "SPX", "ts": ts, "spot": 7490.0, "sigma": 10.0,
            "reversion_extreme": {"fired": gates_fired,
                                  "direction": direction if gates_fired else None},
            "watchtower": wt}


def test_watchtower_report_card_grades_and_splits_disagreements():
    rows = [
        _wt_row(0, gates_fired=True, tower_fired=True),          # both fired (call)
        _wt_row(60, gates_fired=False, tower_fired=True,
                tower_dir="put"),                                 # tower-only (put)
        _wt_row(120, gates_fired=True, tower_fired=False),        # gates vetoed by tower
        _wt_row(180, gates_fired=False, tower_fired=False),       # both passed
    ]
    # bars: every long target (7493) touched quickly; the put fire wins too (7487)
    bars = [{"ts": (T0 + timedelta(minutes=m)).isoformat(),
             "high": 7494.0, "low": 7486.0, "close": 7490.0} for m in (5, 65, 125, 185)]
    res = pab.grade_day("2026-07-01", rows=rows,
                        bars_lookup=lambda t: bars if t == "SPX" else [])
    wt = res["watchtower"]
    sc = wt["scans"]
    assert sc["judged"] == 4 and sc["both_fired"] == 1 and sc["tower_only"] == 1
    assert sc["gates_vetoed"] == 1 and sc["both_passed"] == 1
    # ambiguous bars (both lines in one bar) grade pessimistically as losses,
    # so every decided trade lands in the tallies — the split must add up
    assert wt["disagree"]["tower_only"]["n"] == 1
    assert wt["disagree"]["gates_vetoed"]["n"] == 1
    assert wt["n"] == 2                                # tower's own fires graded
    assert "total_r" in wt and "median_r" in wt


def test_watchtower_card_segments_solo_from_contested_and_records_inference():
    rows = [  # direction flip between them so the resolver's same-dir dedup keeps both
        _wt_row(0, gates_fired=False, tower_fired=True, tower_dir="call",
                interest="gap_stretch -0.80σ", scene="stretched, fading up"),   # contested
        _wt_row(60, gates_fired=False, tower_fired=True, tower_dir="put",
                interest="hourly heartbeat", scene="quiet, drifting down"),     # solo
    ]
    bars = [  # the call fire wins at :05, the put fire wins at :65
        {"ts": (T0 + timedelta(minutes=5)).isoformat(),  "high": 7494.0, "low": 7489.0, "close": 7493.0},
        {"ts": (T0 + timedelta(minutes=65)).isoformat(), "high": 7491.0, "low": 7486.0, "close": 7488.0},
    ]
    res = pab.grade_day("2026-07-01", rows=rows,
                        bars_lookup=lambda t: bars if t == "SPX" else [])
    wt = res["watchtower"]
    # graded apart by wake population
    assert wt["by_interest"]["contested"]["n"] == 1
    assert wt["by_interest"]["solo"]["n"] == 1
    assert wt["scans"]["contested"] == 1 and wt["scans"]["solo"] == 1
    # each graded trade records the tower's inference AND what happened
    solo_trade = wt["by_interest"]["solo"]["trades"][0]
    assert solo_trade["scene"] == "quiet, drifting down"
    assert solo_trade["population"] == "solo"
    assert solo_trade["interest"] == "hourly heartbeat"
    assert solo_trade["outcome"] in ("win", "loss", "scratch")
