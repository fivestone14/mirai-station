...
| Reasoner | Watchtower (`wt-11`), blind vote then reveal, its own memory in `state/spx_rag` | `sndk_read` (`strikes-1`), reads the Strikes Payload cold, its own memory in `state/sndk_rag` |
| Scans land in | `state/reversion/` | `state/sndk_reversion/`, with the minute bars in `state/sndk_bars/` and the side packets in `state/sndk_side/` |
| Documents per scan | One: the scene | **Three**: the Strikes Payload the model reads (`sndk_board`), the legacy Scene Payload still built for the wake gate and the memory (`sndk_read`, deprecated 2026-09-05), and the bar-anchored side packet (`side-1`) — the last two are records and displays, never a prompt |
...
| **SNDK payload** | What the reading model is actually handed: the Strikes Payload JSON, rebuilt live through `sndk_board.build_scene_v2`, with the wake-gate thresholds read off the reader's own constants. The payload segment is a dropdown: the Strikes Payload (sent to the model), the Scene Payload v5 (legacy, deprecated, still built for the gate) and the Gate Payload (the old magnet and walls, store only). The Pipeline Architecture button opens the pipeline replayed on a recorded session (every scan, book, bar and payload firing at the moment it did, with a clock, a spike strip and the calls table) above the component cards. Its Memory view reads the model's RAG store through the same CLI the model uses. |
...
- `GET /api/sndk/pipeline?day=YYYY-MM-DD&user=will` and `GET /api/sndk/pipeline/days?user=will` — the Pipeline Architecture replay: a recorded session's scans, distinct books, minute bars and read rows as minutes since the open, derived from the rows' own clocks; served to `static/pipeline.html`, which the modal embeds.
- `GET /api/sndk/payload?user=will` — the SNDK Payload tab: since 09-05 the Strikes Payload JSON `sndk_board.build_scene_v2` hands the model, with `legacy` (the Scene Payload v5, labelled deprecated) and `gate_payload` (the old magnet and walls) beside it and `payload` naming which one the reader is running; before that, the exact scene JSON `sndk_read.build_scene` hands the model (rebuilt live through the same functions) + the user-message wrapper; 403 unless `user` names the permitted user (`MIRAI_PAYLOAD_USER`, default `will`) or a front door forwards that name — a route-level name check, never auth. The page's matching UI lock (name box + Unlock) was removed 08-22: the tab now opens straight into the live scene, because nothing reaches it that has not already come through the front door. Since 09-03 the same response also carries `side`, the bar-anchored packet rebuilt through `sndk_side`'s own builder — a sibling of `scene`, never a child, because `user_prompt` and `scene_chars` are pinned to the scene alone. A packet that cannot be built comes back as `null` and the scene renders unaffected.
